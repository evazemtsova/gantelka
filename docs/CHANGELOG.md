...
- При запуске сессии тренировки X, если есть прошлые сессии этой же тренировки — поля «раз»/«кг» подгружаются из самой свежей. Пользователь правит цифры, а не заполняет с нуля. Done-статус не префилится (зачёркивания нет — упражнение выглядит как новое).
- Логика: `useSessions()` → найти первую (самую свежую) сессию с `workoutId === current.id` → смапить её `exercises[i].sets` в `setsByEx`, числа `number | null` → строки для инпутов. Упражнения, которых не было в прошлой сессии — пустые как раньше.
